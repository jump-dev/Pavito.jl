...
function MOI.optimize!(model::Optimizer)
    ...
    if (!isnothing(model.nlp_block) && model.nlp_block.has_objective) ||
        ...
        if isnothing(model.nlp_obj_var)
            ...
            MOI.set(model.mip_optimizer,
                MOI.ObjectiveFunction{VI}(), model.nlp_obj_var)
        end
    else
        if !isnothing(model.nlp_obj_var)
    ...
    if model.log_level > 0
        obj_type = ((!isnothing(model.nlp_block) &&
            model.nlp_block.has_objective) ? "nonlinear" :
            (model.objective isa SQF ? "quadratic" : "linear"))
        ncont = length(model.cont_variables)
        nint = length(model.int_indices)
        nnlp = (isnothing(model.nlp_block) ? 0 :
    ...
    if isnothing(model.nlp_block)
        ...
        MOI.initialize(model.nlp_block.evaluator, intersect([:Grad, :Jac, :Hess],
    ...
    if ini_nlp_status in [MOI.OPTIMAL, MOI.LOCALLY_SOLVED, MOI.ALMOST_OPTIMAL]
        cont_solution = MOI.get(model.cont_optimizer, MOI.VariablePrimal(),
    ...
    elseif ini_nlp_status == MOI.NORM_LIMIT
        ...
        if ini_nlp_status in [MOI.INFEASIBLE, MOI.LOCALLY_INFEASIBLE,
    ...
    if isempty(model.int_indices)
        ...
        if !isnothing(cont_solution) && MOI.supports(model.mip_optimizer,
            MOI.VariablePrimalStart(), VI) &&
            all(isfinite, cont_solution)
            MOI.set(model.mip_optimizer, MOI.VariablePrimalStart(),
                model.mip_variables, cont_solution)

            if !isnothing(model.nlp_obj_var)
                MOI.set(model.mip_optimizer, MOI.VariablePrimalStart(),
                    model.nlp_obj_var, cont_obj)
            end
        end
        ...
        if model.mip_solver_drives
            ...
            function lazy_callback(cb)
                ...
                model.mip_solution = MOI.get(model.mip_optimizer,
                    MOI.CallbackVariablePrimal(cb), model.mip_variables)
                round_mipsol = round.(model.mip_solution)
                if any(i -> abs(model.mip_solution[i] - round_mipsol[i]) > 1e-5,
                    model.int_indices)
                ...
                if all(isfinite, cont_solution)
                    add_cuts(model, cont_solution, jac_IJ, jac_V, grad_f,
                        is_max, cb)
                else
            ...
            MOI.set(model.mip_optimizer, MOI.LazyConstraintCallback(),
                lazy_callback)
            ...
            function heuristic_callback(cb)
                ...
                if model.new_incumb
                    if isnothing(model.nlp_obj_var)
                        vars = model.mip_variables
                        incumb = model.incumbent
                    else
                        vars = vcat(model.mip_variables, model.nlp_obj_var)
                        incumb = vcat(model.incumbent, model.objective_value)
                    end
                    MOI.submit(model.mip_optimizer, MOI.HeuristicSolution(cb),
                        vars, incumb)
                    model.new_incumb = false
                end
            end

            MOI.set(model.mip_optimizer, MOI.HeuristicCallback(),
                heuristic_callback)

            if isfinite(model.timeout)
                MOI.set(model.mip_optimizer, MOI.TimeLimitSec(),
            ...
            model.objective_bound = MOI.get(model.mip_optimizer,
                MOI.ObjectiveBound())
            update_gap(model, is_max)
        else
            # iterative method
            ...
            while (time() - start) < model.timeout
                ...
                if (mip_status == MOI.INFEASIBLE) ||
                    (mip_status == MOI.INFEASIBLE_OR_UNBOUNDED)
                    model.status = MOI.INFEASIBLE
                    break
                elseif (mip_status != MOI.OPTIMAL) &&
                    (mip_status != MOI.ALMOST_OPTIMAL)
                    @warn "MIP solver status was $mip_status, terminating Pavito"
                    model.status = mip_status
                    break
                end
                model.mip_solution = MOI.get(model.mip_optimizer,
                    MOI.VariablePrimal(), model.mip_variables)

                # update best bound from MIP bound
                mip_obj_bound = MOI.get(model.mip_optimizer, MOI.ObjectiveBound())
                if isfinite(mip_obj_bound) &&
                    comp(model.objective_bound, mip_obj_bound)
...
function update_gap(model::Optimizer, is_max::Bool)
    ...
    if isfinite(model.objective_value) && isfinite(model.objective_bound)
        model.objective_gap = (model.objective_value - model.objective_bound) /
...
function solve_subproblem(model::Optimizer, comp::Function)
    fix_int_vars(model.cont_optimizer, model.cont_variables,
        model.mip_solution, model.int_indices)
    MOI.optimize!(model.cont_optimizer)
    ...
    if MOI.get(model.cont_optimizer, MOI.PrimalStatus()) == MOI.FEASIBLE_POINT
        ...
        nlp_solution = MOI.get(model.cont_optimizer, MOI.VariablePrimal(),
            model.cont_variables)
    ...
    if (!isnothing(model.nlp_block) && !isempty(model.nlp_block.constraint_bounds)
        ...
        if !isnothing(model.nl_slack_variables)
            obj = MOI.ScalarAffineFunction([MOI.ScalarAffineTerm(1.0, x) for
                x in model.nl_slack_variables], 0.0)
        else
            obj = MOI.ScalarAffineFunction(
                Array{MOI.ScalarAffineTerm{Float64}}([]), 0.0)
        end
        ...
        if !isnothing(model.nlp_block) &&
            ...
            model.infeasible_evaluator = InfeasibleNLPEvaluator(
                model.nlp_block.evaluator, length(model.infeasible_variables),
                falses(length(model.nlp_block.constraint_bounds)))
            model.nl_slack_variables = MOI.add_variables(
                _infeasible(model), length(model.nlp_block.constraint_bounds))
            ...
            MOI.set(_infeasible(model), MOI.NLPBlock(), MOI.NLPBlockData(
                bounds, model.infeasible_evaluator, false))
        end

        # we need to add quadratic variables afterwards as `InfeasibleNLPEvaluator`
        # assumes the original variables are directly followed by the NL slacks
        if !isempty(model.quad_LT)
            model.quad_LT_slack = MOI.add_variables(
                _infeasible(model), length(model.quad_LT))
            model.quad_LT_infeasible_con =
                map(eachindex(model.quad_LT)) do i
                    (func, set) = model.quad_LT[i]
                    new_func = MOI.Utilities.operate(-, Float64, func,
                        model.quad_LT_slack[i])
                    MOI.add_constraint(_infeasible(model), new_func, set)
                end
        ...
        if !isempty(model.quad_GT)
            model.quad_GT_slack = MOI.add_variables(
                _infeasible(model), length(model.quad_GT))
            model.quad_GT_infeasible_con =
                map(eachindex(model.quad_GT)) do i
    ...
    fix_int_vars(model.infeasible_optimizer, model.infeasible_variables,
        model.mip_solution, model.int_indices)
    MOI.set(_infeasible(model), MOI.VariablePrimalStart(),
        model.infeasible_variables, model.mip_solution)
    ...
    if !isnothing(model.nlp_block) && !isempty(model.nlp_block.constraint_bounds)
        ...
        for i in eachindex(model.nlp_block.constraint_bounds)
            ...
            val = (model.infeasible_evaluator.minus[i] ?
                (g[i] - bounds.upper) : (bounds.lower - g[i]))
            # sign of the slack changes if the constraint direction changes
            MOI.set(_infeasible(model), MOI.VariablePrimalStart(),
                model.nl_slack_variables[i], max(0.0, val))
        end
    end

    for i in eachindex(model.quad_LT)
        val = eval_func(model.mip_solution, model.quad_LT[i][1]) -
            model.quad_LT[i][2].upper
        MOI.set(_infeasible(model), MOI.VariablePrimalStart(),
            model.quad_LT_slack[i], max(0.0, val))
    end

    for i in eachindex(model.quad_GT)
        val = model.quad_GT[i][2].lower -
            eval_func(model.mip_solution, model.quad_GT[i][1])
        MOI.set(_infeasible(model), MOI.VariablePrimalStart(),
            model.quad_GT_slack[i], max(0.0, val))
    end
    ...
    return MOI.get(model.infeasible_optimizer, MOI.VariablePrimal(),
    ...
    if model.log_level >= 1
        ...
        if model.objective_gap < 1000
            @printf("%5d | %+14.6e | %+14.6e | %11.3e | %11.3e\n",
                model.num_iters_or_callbacks, model.objective_value,
                model.objective_bound, model.objective_gap, (time() - start))
        else
            obj_gap = (isnan(model.objective_gap) ? "Inf" : ">1000")
            @printf("%5d | %+14.6e | %+14.6e | %11s | %11.3e\n",
                model.num_iters_or_callbacks, model.objective_value,
                model.objective_bound, obj_gap, (time() - start))
        end
        flush(stdout)
        flush(stderr)
    end
...
eval_func(values, func) = MOI.Utilities.eval_variables(
    vi -> values[vi.value], func)
...
function _bound_set(lb::T, ub::T) where T
    if _has_upper(ub)
        if _has_lower(lb)
            error("An NLP constraint has lower bound $lb and upper bound $ub " *
